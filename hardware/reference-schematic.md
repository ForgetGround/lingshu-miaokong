...
当前硬件选型按原理图设计整理，主控为 `ESP32-S3-WROOM-1-N16R8` 模块。
...
FPC 连接器包含 `EMG1` 到 `EMG8`、`GND` 和 `+5V` 网络。仓库文档按 8 路 EMG 输入整理。

ADC/EMG 前端由 `AD8226ARZ-R7` 仪表放大器和 `TL084CDT` 四运放构成，使用 `+9V`、`-9V` 双电源，末端输出网络名为 `OUT`。详见 `hardware/adc-amplifier.md`。

打样前需要从 EasyEDA 源文件导出 netlist，确认每个 `EMGx` 连接到 ESP32-S3 的具体 ADC GPIO，并补充输入保护、RC 滤波和前端输出电压范围。由于放大器运行在 `±9V` 供电下，进入 ESP32-S3 ADC 前必须确认信号已经转换并限制在 MCU 允许的 ADC 电压范围内。

## 5 通道关节驱动

“5通道关节驱动”按 `Finger1` 到 `Finger5` 分组。每个 finger 下有 3 个输出级，网络命名呈现为：
...
控制侧对应 `IO1_1` 到 `IO5_3`。每个输出级包含开关器件、SS34 二极管、LED 指示和 3.3 kOhm 电阻。
